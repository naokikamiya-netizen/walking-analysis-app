# ==========================================================
#  app.py - 歩行分析アプリ (真・最終完成版 - 自動回転＆フォント直指定)
# ==========================================================
...
# --- メインの分析ロジック ---
def analyze_walking(video_path, progress_bar, status_text): # ★★★ should_rotate引数を削除 ★★★
    ...
    auto_rotate = frame_h > frame_w  # 縦長動画ならTrue
    ...
    for frame_count in range(total_frames):
        ...
        if auto_rotate: image = cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE) # ★★★ 自動回転を適用 ★★★
    ...
    for i in range(total_frames):
        ...
        if i < len(all_landmarks) and all_landmarks[i]:
            mp_drawing.draw_landmarks(image, all_landmarks[i], mp_pose.
