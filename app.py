# ==========================================================
#  app.py - 歩行分析アプリ (最終完成版 - 静的・動的分離モデル)
# ==========================================================
...
def analyze_walking(video_path, progress_bar, status_text, should_rotate):
    ...
    # ★★★ 静的・動的評価ロジック ★★★
    num_static_frames = min(int(fps * 1.0), 30) # 最初の1秒(最大30フレーム)を静的評価期間とする
    if len(angles_np) <= num_static_frames: return None, None # 動画が短すぎる場合
    ...
    # 0度より上(左肩下がり)と下(右肩下がり)の全ての点の平均を計算
    ...
    for i in range(total_frames):
        ...
        ax_g.plot(time_stamps, angles_np, color='#00FFFF', lw=2) # ありのままのデータをプロット
        ...
        ax_g.set_ylim(-y_limit, y_limit); ax_g.grid(True, linestyle=':', color='gray', alpha=0.7)
...
def display_results():
    ...
    col1.metric(label="動的傾斜 (歩行中の揺れ・右)", value=f"{abs(summary['avg_right_down_dynamic']):.2f} 度")
    col2.metric(label="動的傾斜 (歩行中の揺れ・左)", value=f"{summary['avg_left_down_dynamic']:.2f} 度")
